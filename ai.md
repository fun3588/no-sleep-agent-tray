...
- [x] ~~发第一个 GitHub Release~~ — v2.1.1 已发布（workflow 一次跑通，`keepawake-x64.exe` + `keepawake-arm64.exe` 附件齐全）
  First GitHub Release published (v2.1.1, both assets attached).
- [x] ~~`keepawake.go` 是 v1 时代的纯控制台单文件版~~ — 已删除（托盘版只有 Rust），v2.1.2
  Deleted legacy `keepawake.go` in v2.1.2.
- [ ] 在真笔记本上验证“电源方案”调光路（当前只在台式机/虚拟机上验证了 WMI 路）
- [ ] 英文界面超长字符串的人工目检（已加宽到 496px + 程序化 dump 复核，缺真机截图确认）
  Manual eyeball check of long EN strings (dialog widened to 496px + programmatic dump verified; no real screenshot yet).
...
- 2026-09-14：清理——删除过时的 `keepawake.go`，精简 `main()` 里冗余的 `wants_output` 中转变量；版本 2.1.2，双架构重编 push。另记：`Get-Process keepawake` 匹配不到改名运行的 `keepawake-arm64`/`keepawake-x64` 进程，杀进程要用 `keepawake*` 通配，否则 dist 下的 exe 会被占住无法覆盖。
  Cleanup: deleted legacy `keepawake.go`, removed redundant `wants_output`; v2.1.2 rebuilt. Note: kill tray processes with `keepawake*` wildcard, renamed exes won't match `keepawake`.
- 2026-09-14：两个 MD 定稿——`README.md` / `ai.md` 全文双语（先中文后英文）；v2.1.1 Release 发布成功；仓库历史清零为单初始提交后 force-push（`v2.1.1` tag 与 Release 不动，仍指向旧提交）。
  Both MDs finalized bilingual (CN first); v2.1.1 released; history squashed to a single initial commit and force-pushed (`v2.1.1` tag/Release untouched).
```text
no-sleep-agent-tray/
